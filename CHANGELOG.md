...
- **「更新并重启」不再误报失败**：apply / rollback 会让进程 2 秒后自杀重启，那一刻前端 HTTP 连接被切断、axios 报无响应错误，旧逻辑误弹「更新失败」。现在把这种连接中断视为预期的「服务正在重启」，进入重启等待态并轮询 `update/check` 直到新版本回来（最多 ~60s），完成后提示「已更新并重启完成」。apply 请求超时从 15s 提到 200s，避免下载二进制耗时较长时被提前切断。
- **release 更新内容不再为空**：release.yaml 生成 notes 原本只按 `## [VERSION]` 标题抽取 CHANGELOG，未命中时退化成一句英文占位。现增加 fallback：未命中版本标题时自动抽取第一个 `### YYYY-MM-DD` 日期小节作为更新内容。

## [0.6.8] - 2026-06-14

主题：**并发调度监控弹窗人话化 + release workflow 解耦 Docker**。
